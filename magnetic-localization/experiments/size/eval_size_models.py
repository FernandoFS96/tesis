# ============================================================
# eval_size_models.py  (with optional extrapolation evaluation)
# ============================================================
"""
Evalúa 6 modelos sobre los datasets de “tamaño” (prof. 20 m, 4 sensores):

  • MLP_2x1m, MLP_4x2m, MLP_8x4m
  • DRS_size  (combinado)
  • DRS_general, ANP     (pre-entrenados)

Genera:
  • mae_size_tests.csv           ── o  mae_size_tests_OOD.csv
  • heatmap_size_tests.png       ── o  heatmap_size_tests_OOD.png

Opcional:
  --ood-csv           CSV extra fuera de distribución (p. ej. 20m-64T_20mx10m)
  --full-traj         MAE del ANP sobre toda la trayectoria
  --extrapolation-eval  Usa contextos secuenciales (primeros k puntos)
  --ctx-size <int>      (con extrapolation) fija el número de puntos de contexto

Ejemplo:
    python eval_size_models.py \
      --data-root /home/fernando/tesis/ANP/data/tasks/3x3x3 \
      --mlp-dir   /home/fernando/tesis/ANP/training/drs_size_runs \
      ...
      --ood-csv /home/fernando/tesis/ANP/data/tasks/1x1x1/20m-64T_20mx10m/dataset_without_sensor_1_3.csv \
      --device cuda --batch-size 10

    python eval_size_models.py \
      --data-root /home/fernando/tesis/ANP/data/tasks/3x3x3 \
      --mlp-dir   /home/fernando/tesis/ANP/training/drs_size_runs \
      --anp-ckpt  /home/fernando/tesis/ANP/training/anp_runs/no_sensor_mask/extrapolation/anp_best_ep7223.pt \
      --drs-general-ckpt /home/fernando/tesis/ANP/training/drs_runs/drs_general_best_ep7958.pt \
      --ood-csv /home/fernando/tesis/ANP/data/tasks/1x1x1/20m-64T_20mx10m/dataset_without_sensor_1_3.csv \
...
# ─────────────────────────── main ───────────────────────────
def main():
    ap = argparse.ArgumentParser(description="Eval size-test (6×3 [+ OOD + extrapolation])")
    ap.add_argument("--data-root", required=True)
    ap.add_argument("--mlp-dir",   required=True,
                    help="drs_size_runs con subdirs 2x1m/ 4x2m/ 8x4m/ combined/")
    ...
    ap.add_argument("--out-dir", default="size_eval_results")
    ap.add_argument("--ood-csv", type=Path,
                    help="CSV extra fuera de distribución")
    ap.add_argument("--full-traj", action="store_true",
                    help="MAE ANP sobre TODA la trayectoria")
    ...
    if args.ood_csv:
        ...
        ood_label = args.ood_csv.parent.name.split("_")[0]
        csvs[ood_label] = args.ood_csv
    ...
    mlp_paths = {
        "MLP_2x1m":  next((Path(args.mlp_dir)/"2x1m").glob("MLP_*_best_ep*.pt")),
        "MLP_4x2m":  next((Path(args.mlp_dir)/"4x2m").glob("MLP_*_best_ep*.pt")),
        "MLP_8x4m":  next((Path(args.mlp_dir)/"8x4m").glob("MLP_*_best_ep*.pt")),
        "DRS_size":  next((Path(args.mlp_dir)/"combined").glob("DRS_size_best_ep*.pt")),
    }
    ...
    for folder, csv_path in csvs.items():
        ...
        # 2) ANP
        class SingleCSV(torch.utils.data.Dataset):
        ...
        if args.full_traj:
            ds = SingleCSV(csv_path, use_meta=args.use_meta)
            mae_vals = [mae_anp_fulltraj(anp, s, c, device) for s, c in ds]
            res["ANP"] = sum(mae_vals) / len(mae_vals)
        else:
            loader = DataLoader(
                SingleCSV(csv_path, use_meta=args.use_meta),
    ...
    if args.ood_csv:
        csv_out = out_dir / "mae_size_tests_OOD.csv"
        png_out = out_dir / "heatmap_size_tests_OOD.png"
    else:
        csv_out = out_dir / "mae_size_tests.csv"
        png_out = out_dir / "heatmap_size_tests.png"

    df.T.to_csv(csv_out, float_format="%.6f")
    print(f"✓ CSV {csv_out}")

    # ---- heatmap ----
    heatmap(df, png_out)
    print("✓ Size-test completado")
