...
python src/evaluation/evaluate.py \
  --data-path /home/fernando/tesis/underwater-localization/data/data_save/data_processed_topologies_low_variance/topology_ellipsoidal/val_data.pkl \
...
class Evaluator:
    ...
    def eval_anp_mean(self) -> Dict[int, List[float]]:
        ...
        return {c: [float(np.mean(e[f"ANP({c}%)"])) for e in errs] for c in CONTEXTS}

    def eval_mlp_mean(self) -> Dict[str, List[float]]:
        errs = self._compute_errors()
        return {name: [float(np.mean(e[name])) for e in errs] for name in self.mlps.keys()}
    def eval_heatmap(self) -> tuple[Dict[str, List[float]], Dict[int, List[float]]]:
        errs = self._compute_errors()
        mlp_maes = {name: [float(np.mean(e[name])) for e in errs] for name in self.mlps.keys()}
        anp_maes = {c: [float(np.mean(e[f"ANP({c}%)"])) for e in errs] for c in CONTEXTS}
