...
class Config:
    ...
    batch_size: int = 16   # episodios por batch — ajustar según VRAM disponible
...
def make_episode_fixed(
    ...
    y: pd.DataFrame,
    ctx_frac: float,
    n_ctx: int,      # SIEMPRE exactamente este número de puntos de contexto
    n_tgt: int,      # SIEMPRE exactamente este número de puntos de target
    device: torch.device,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Versión batch-compatible de make_episode.
    Samplea EXACTAMENTE n_ctx y n_tgt puntos — tamaño fijo para poder apilar.

    Contexto = prefijo causal (primeras ctx_frac*T filas), submuestreado a n_ctx.
    Target   = toda la trayectoria, submuestreada a n_tgt (incluye contexto).
    """
    T = len(X)
    N_c = max(1, int(ctx_frac * T)) # nº real de filas de contexto disponibles
    N_c = min(N_c, T) # no puede superar T

    # Contexto: prefijo causal submuestreado a exactamente n_ctx puntos
    # np.random.choice SIN replace garantiza puntos únicos
    ctx_pool = np.arange(N_c)
    ctx_idx  = np.sort(np.random.choice(ctx_pool, size=n_ctx, replace=len(ctx_pool) < n_ctx))

    # Target: toda la trayectoria submuestreada a exactamente n_tgt puntos
    tgt_pool = np.arange(T)
    tgt_idx  = np.sort(np.random.choice(tgt_pool, size=n_tgt, replace=T < n_tgt))

    X_vals = torch.tensor(X.values, dtype=torch.float32)
    y_vals = torch.tensor(y.values, dtype=torch.float32)

    # Shape: (N, D) — sin dimensión de batch todavía, se apila fuera
    ctx_x = X_vals[ctx_idx]   # (n_ctx, D)
    ctx_y = y_vals[ctx_idx]   # (n_ctx, O)
    tgt_x = X_vals[tgt_idx]   # (n_tgt, D)
    tgt_y = y_vals[tgt_idx]   # (n_tgt, O)

    return ctx_x, ctx_y, tgt_x, tgt_y

def make_batch(
    tasks: list,                  # lista de (X_df, y_df) ya ordenados
    batch_size: int,
    ctx_frac: float,
    n_ctx: int,
    n_tgt: int,
    ...
    """
    Construye un batch de batch_size episodios apilados en dim=0.
    Samplea las tareas CON reemplazo para poder pedir cualquier batch_size.
    ...
    Returns:
        ...
        target_y   (B, n_tgt, O)
    """
    batch_ctx_x, batch_ctx_y = [], []
    batch_tgt_x, batch_tgt_y = [], []

    # Samplear batch_size tareas (con reemplazo — válido en meta-learning)
    chosen = random.choices(tasks, k=batch_size)

    for X, y in chosen:
        cx, cy, tx, ty = make_episode_fixed(
            X, y, ctx_frac, n_ctx, n_tgt, device
        )
        batch_ctx_x.append(cx)
        batch_ctx_y.append(cy)
        batch_tgt_x.append(tx)
        batch_tgt_y.append(ty)

    return (
        torch.stack(batch_ctx_x).to(device),   # (B, n_ctx, D)
        torch.stack(batch_ctx_y).to(device),   # (B, n_ctx, O)
        torch.stack(batch_tgt_x).to(device),   # (B, n_tgt, D)
        torch.stack(batch_tgt_y).to(device),   # (B, n_tgt, O)
    )
# ══════════════════════════════════════════════════════════════════════════════
# MÉTRICAS
# ══════════════════════════════════════════════════════════════════════════════
...
def denorm_mae(pred_mean, target_y, denorm_values, target_cols):
    ...
    for i, col in enumerate(target_cols):
        ...
        result[col] = (pred_dn - true_dn).abs().mean().item() # scale MAE back to original units
    return result
...
def evaluate(
    ...
    for ctx_frac in cfg.val_ctx_fracs:
        ...
        for X, y in tasks:
            ...
            ctx_x_list.append(cx); ctx_y_list.append(cy)
            tgt_x_list.append(tx); tgt_y_list.append(ty)

        ctx_x = torch.stack(ctx_x_list).to(device)
        ctx_y = torch.stack(ctx_y_list).to(device)
        tgt_x = torch.stack(tgt_x_list).to(device)
        tgt_y = torch.stack(tgt_y_list).to(device)
...
def train(cfg: Config):
    ...
    for epoch in pbar:
        ...
        steps_per_epoch = cfg.episodes_per_epoch // cfg.batch_size  # same total episodes, but in batches
        for _ in range(steps_per_epoch):
            # random fraction of context for this episode (between ctx_min_frac and ctx_max_frac)
            ctx_frac = random.uniform(cfg.ctx_min_frac, cfg.ctx_max_frac)
            ctx_x, ctx_y, tgt_x, tgt_y = make_batch(
                train_sorted,
                batch_size=cfg.batch_size,
                ctx_frac=ctx_frac,
                n_ctx=cfg.max_context_pts,
                n_tgt=cfg.max_target_pts,
                device=device,
            )
...
def parse_args():
    ...
    p.add_argument("--max_ctx_pts",  type=int,   default=2000, dest="max_context_pts")
    p.add_argument("--max_tgt_pts",  type=int,   default=2000, dest="max_target_pts")
